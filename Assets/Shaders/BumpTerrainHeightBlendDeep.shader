Shader "MyTerrain/BumpTerrainHeightBlendDeep"
{
   Properties{
		...
		SubShader
			{
				...
				float _GlobalSnowAmountDeep, _TerrainSnowBlendStrength, _TerrainSnowBlendPower;
				...
				float4 _snowColorUpper, _snowColorBottom;
					...
					void vert(inout appdata v)
					{
						...
						if (splat_control.r > touchBend.r) {
							...
							_snowDispMap = (splat_control.r - touchBend.r) * _GlobalSnowAmountDeep * UNITY_SAMPLE_TEX2DARRAY_LOD(_Array1, float3(uv / float2(_TileSizeArray[0].x, _TileSizeArray[0].y) + float2(_TileSizeArray[0].z, _TileSizeArray[0].w), 0), 3).r * _BumpScaleArray[0];
						}
